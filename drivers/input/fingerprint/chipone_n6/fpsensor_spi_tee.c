...
//+bug 518763,gjx.wt,20191204,modify,check the retrun value
    if(fb_register_client(&fpsensor_dev->notifier))
        fpsensor_debug(INFO_LOG,"%s fb_register_client err",__func__);
//-bug 518763,gjx.wt,20191204,modify,check the retrun value
#endif
...
static int __init fpsensor_init(void)
{
    ...
    if (egis_fp_exist) {
        fpsensor_debug(ERR_LOG, "%s, fpsensor egis sensor has been detected.\n", __func__);
         return -EINVAL;
    }

//+oa 6648421,gjx.wt,add,20200915,fingertech fingerprint bringup
    if (fingtech_fp_exist) {
        fpsensor_debug(ERR_LOG, "%s, fingtech sensor has been detected.\n", __func__);
         return -EINVAL;
    }
//-oa 6648421,gjx.wt,add,20200915,fingertech fingerprint bringup

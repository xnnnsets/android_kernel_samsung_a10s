...
#endif
#if (SF_PROBE_ID_EN && !SF_RONGCARD_COMPATIBLE ) || TRUSTONIC_TEE
...
bool sunwave_fp_exist = false;
struct spi_device *spi_fingerprint;
int fpc_or_chipone_exist = -1;
bool egis_fp_exist = false;
bool fingtech_fp_exist = false;//oa 6648421,gjx.wt,add,20200915,fingertech fingerprint bringup
...
spi_fingerprint=sf_ctl_dev.pdev ;
...
if(err<0){
	sunwave_fp_exist=false;
	sf_ctl_dev.free_gpio(&sf_ctl_dev);
	return 0;
	}
	
    sunwave_fp_exist=true;
...
//        input_unregister_device(sf_ctl_dev.input);
 ...
 //       input_unregister_device(sf_ctl_dev.input);
    ...
    err = fb_register_client(&sf_ctl_dev.notifier);
    if(err){
	xprintk(KERN_ERR,"FAILED TO REGISTER FB CLIENT (%d)\n", err);
	return err;
    }
#endif
    /* beanpod ISEE2.7 */

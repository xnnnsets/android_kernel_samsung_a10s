...
#define FPC1510_CHIP                  0x1000 //0x10xx
...
static int check_hwid(struct spi_device *spi)
{
	...
	do {
		...
		if(( hardware_id & FPC1022_CHIP_MASK_SENSOR_TYPE) == FPC1510_CHIP)
		{
...
static int fpc1022_platform_probe(struct platform_device *pldev)
{
    ...
    if (!fpc) {
        dev_err(dev,"failed to allocate memory for struct fpc1022_data\n");
        ret = -ENOMEM;
        goto exit;
    }
	...
	if (node_eint == NULL) {
		rc = -EINVAL;
		FPC_ALOGE("mediatek,fpsensor_fp_eint_n6 cannot find node_eint rc = %d.\n", rc);
		goto exit;
	}
...
//+oa 6648421,gjx.wt,add,20200915,fingertech fingerprint bringup
    if (fingtech_fp_exist) {
        printk(KERN_ERR "%s fingtech sensor has been detected, so exit FPC sensor detect.\n",__func__);
        return -EINVAL;
    }
//-oa 6648421,gjx.wt,add,20200915,fingertech fingerprint bringup

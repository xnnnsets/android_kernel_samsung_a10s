    ...
    u32 hwid, u16 fwid, u16 curr_firmware_ver)
{
    ...
    if (is_filesystem_mounted(CFG_CTS_FIRMWARE_FILEPATH)) {
		//filepath:CFG_CTS_FIRMWARE_FILEPATH => filp_open()
		//filepath:CFG_CTS_FIRMWARE_FILENAME => request_firmware()
...
out:
    ...
    snprintf(firmware_ver,HARDWARE_MAX_ITEM_LONGTH,"TXD,ICNL9911C,FW:0x%04x", FIRMWARE_VERSION(firmware));
    cts_info("firmware_ver is : %s \n",firmware_ver);

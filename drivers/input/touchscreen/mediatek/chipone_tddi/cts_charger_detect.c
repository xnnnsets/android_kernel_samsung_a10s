    ...
    if (ret) {
        ...
        if (psp < 0) {
            ...
            if (psy == NULL ||
                ...
                cts_err("-------psy = %s,%d,%p",psy,psy,psy);
				cts_err("-------POWER_SUPPLY_GET_PROPERTY =%d",POWER_SUPPLY_GET_PROPERTY(psy, psp, &val));
...
#ifdef CONFIG_CTS_OF
   cts_data->device->of_node = of_find_compatible_node(NULL, NULL, "chipone-tddi");

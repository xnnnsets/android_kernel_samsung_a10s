...
struct pinctrl *i2c2_pinctrl;
struct pinctrl *i2c4_pinctrl;
struct pinctrl_state *i2c2_sda_pin_en0;
struct pinctrl_state *i2c2_sda_pin_en1;
struct pinctrl_state *i2c4_sda_pin_en0;
struct pinctrl_state *i2c4_sda_pin_en1;
...
imgsensor_sensor_open(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (psensor_func &&
		...
		//bug526695,yangzheng.wt,modify,20200203,N6 P to Q,subcamera cannot be opened
		#ifndef CONFIG_MTK_FLASHLIGHT_LED191	//S96716 or S96717 flashlight used for CONFIG_MTK_FLASHLIGHT_MT6370
		//bug520788,wanglei6.wt,modify,20191207,camera cannot be opened
		ret = pinctrl_select_state(i2c2_pinctrl, i2c2_sda_pin_en1);
		if(ret < 0){
			pr_err("=== select i2c2_sda_pin_en1 failed ===\n");
			return -1;
		}

		ret = pinctrl_select_state(i2c4_pinctrl, i2c4_sda_pin_en1);
		if(ret < 0){
			pr_err("=== select i2c2_sda_pin_en1 failed ===\n");
			return -1;
		}
...
imgsensor_sensor_close(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (psensor_func &&
		...
		#ifndef CONFIG_MTK_FLASHLIGHT_LED191	//S96716 or S96717 flashlight used for CONFIG_MTK_FLASHLIGHT_MT6370
		//bug520788,wanglei6.wt,modify,20191207,camera cannot be opened
		ret = pinctrl_select_state(i2c2_pinctrl, i2c2_sda_pin_en0);
		if(ret < 0){
			pr_err("=== select i2c2_sda_pin_en0 failed ===\n");
			return -1;
		}

		ret = pinctrl_select_state(i2c4_pinctrl, i2c4_sda_pin_en0);
		if(ret < 0){
			pr_err("=== select i2c4_sda_pin_en0 failed ===\n");
			return -1;
		}
		#endif
...
//+bug494668 chenbocheng.wt, modify, 2019/11/5, only the correct sensor can power up
    if (err == IMGSENSOR_RETURN_ERROR){
        return err ? -EIO:err;
    }
//-bug494668 chenbocheng.wt, modify, 2019/11/5, only the correct sensor can power up
...
#define STRINGIZE(stringizedName) TOSTRING(stringizedName)
	...
	ret_pinctl = pinctrl_select_state(i2c2_pinctrl, i2c2_sda_pin_en1);
	if(ret_pinctl < 0){
		pr_err("=== select i2c2_sda_pin_en0 failed ===\n");
		return -1;
	}
	ret_pinctl = pinctrl_select_state(i2c4_pinctrl, i2c4_sda_pin_en1);
	if(ret_pinctl < 0){
		pr_err("=== select i2c4_sda_pin_en0 failed ===\n");
		return -1;
	}
	...
	#ifndef CONFIG_MTK_FLASHLIGHT_LED191	//S96716 or S96717 flashlight used for CONFIG_MTK_FLASHLIGHT_MT6370
	//bug520788,wanglei6.wt,modify,20191207,camera cannot be opened
	ret_pinctl = pinctrl_select_state(i2c2_pinctrl, i2c2_sda_pin_en0);
	if(ret_pinctl < 0){
		pr_err("=== select i2c2_sda_pin_en0 failed ===\n");
		return -1;
	}
	ret_pinctl = pinctrl_select_state(i2c4_pinctrl, i2c4_sda_pin_en0);
	if(ret_pinctl < 0){
		pr_err("=== select i2c4_sda_pin_en0 failed ===\n");
		return -1;
	}
...
struct match_hardwareinfo hardwareinfo_camera_name[] = {
  ...
  //+OA6649398,zhouyikuan.wt,ADD,2020/09/08,hi1336_txd_jct sensor bringup
    ...
    //-OA6649398,zhouyikuan.wt,ADD,2020/09/08,hi1336_txd_jct sensor bringup
    //+OA6649409,zhouyikuan.wt,ADD,2020/09/14,hi1336_xl_jct sensor bringup
    ...
    {SENSOR_DRVNAME_N21_DONGCI_MAIN_S5K2P6_MIPI_RAW, "S5K2P6SQ", "dongci"},
    {SENSOR_DRVNAME_N21_TXD_SUB_S5K3L6_MIPI_RAW, "S5K3L6", "tongxingda"},
    {SENSOR_DRVNAME_N21_SHINE_SUB_HI1336_MIPI_RAW, "HI1336", "shengtai"},
    {SENSOR_DRVNAME_N21_HLT_SUB_HI1336_MIPI_RAW, "HI1336", "helitai"},
    {SENSOR_DRVNAME_N21_TXD_WIDE_HI846_MIPI_RAW, "HI846", "tongxingda"},
    ...
    {SENSOR_DRVNAME_N21_CXT_DEPTH_GC02M1B_MIPI_RAW, "GC02M1B_DEPTH", "chengxiangtong"},//bug 530174,gaoyu1.wt,add,20200113,bring up depth gc02m1b
    {SENSOR_DRVNAME_N21_LHYX_DEPTH_BF2253_MIPI_RAW, "BF2253_DEPTH", "lianheyingxiang"},
    ...
    {SENSOR_DRVNAME_N21_QUNH_MICRO_BF2253_MIPI_RAW, "BF2253_MACRO", "qunhui"},
...
static inline int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	for(i = 0; strcmp(hardwareinfo_camera_name[i].psensor_name, "NULL"); i++){
	 ...
	 if(pSensorGetInfo->SensorId == 2){
		 hardwareinfo_set_prop(HARDWARE_BACK_SUBCAM,hardwareinfo_camera_name[i].hardwareinfo_set_name);
		 hardwareinfo_set_prop(HARDWARE_BACK_SUBCAM_MODULEID, hardwareinfo_camera_name[i].sensor_id);	  //set depth back camera sensor id
	 }
...
static int imgsensor_probe(struct platform_device *pdev)
{
	...
	i2c2_pinctrl = devm_pinctrl_get(&pdev->dev);
	if(IS_ERR(i2c2_pinctrl)){
		pr_err("=== i2c_pinctrl pin get failed ===");
		return -1;
	}
	i2c2_sda_pin_en0 = pinctrl_lookup_state(i2c2_pinctrl, "i2c2_pin_sda_pull_en0");
	if (IS_ERR(i2c2_sda_pin_en0)) {
		pr_err("=== i2c_pinctrl i2c2_pin_sda_pull_en0 pin get failed ===");
		return -1;
		}
	i2c2_sda_pin_en1 = pinctrl_lookup_state(i2c2_pinctrl, "i2c2_pin_sda_pull_en1");
	if (IS_ERR(i2c2_sda_pin_en1)) {
		pr_err("=== i2c_pinctrl i2c2_pin_sda_pull_en1 pin get failed ===");
		return -1;
	}
	i2c4_pinctrl = devm_pinctrl_get(&pdev->dev);
	if(IS_ERR(i2c4_pinctrl)){
		pr_err("=== i2c4_pinctrl pin get failed ===");
		return -1;
	}
	i2c4_sda_pin_en0 = pinctrl_lookup_state(i2c4_pinctrl, "i2c4_pin_sda_pull_en0");
	if (IS_ERR(i2c4_sda_pin_en0)) {
		pr_err("=== i2c_pinctrl i2c4_pin_sda_pull_en0 pin get failed ===");
		return -1;
	}
	i2c4_sda_pin_en1 = pinctrl_lookup_state(i2c4_pinctrl, "i2c4_pin_sda_pull_en1");
	if (IS_ERR(i2c4_sda_pin_en1)) {
		pr_err("=== i2c_pinctrl i2c4_pin_sda_pull_en1 pin get failed ===");
		return -1;
	}
